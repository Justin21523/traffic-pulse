...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Ingest public events calendar (concerts/sports/expos) from CSV.")
...
def main() -> None:
    ...
    if not config.sources.calendar.enabled:
        print("[calendar] source disabled (config.sources.calendar.enabled=false)")
        return

    input_path = config.sources.calendar.csv_path
    df = read_csv(input_path)
    normalized = normalize_events_csv(
        df,
        source_name=config.sources.calendar.source_name,
        default_event_type="planned_event",
    )
    out_path = processed_dir / "events_calendar.csv"
    ...
    print(f"[calendar] wrote {out_path} rows={len(normalized):,}")
