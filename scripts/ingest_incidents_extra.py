...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Ingest additional incident/accident feeds from a configured CSV source.")
...
def main() -> None:
    ...
    if not config.sources.incidents.enabled:
        print("[incidents] source disabled (config.sources.incidents.enabled=false)")
        return

    input_path = config.sources.incidents.csv_path
    df = read_csv(input_path)
    normalized = normalize_events_csv(
        df,
        source_name=config.sources.incidents.source_name,
        default_event_type="incident",
    )
    out_path = processed_dir / "events_incidents_extra.csv"
    ...
    print(f"[incidents] wrote {out_path} rows={len(normalized):,}")
